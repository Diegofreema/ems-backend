            ...
            <div class="row">
                ...
                <?=$this->Html->image($settings->logo, ['alt' => 'LOGO', 'class' => 'img-responsive float-left','height'=>100,'width'=>130])?>
                            ...
                            <h1 class="h4 text-gray-900 mb-4"><strong style="font-size: 28px;"><?= $settings->name?></strong><br />
                                ...
                                <b style="font-size: 21px;"> Fee Payment Invoice<br /> </b></h1>
                        
                        </div>
                       
Name : <?=$name ?><br />
Registration Number : NA<br />
 Address :  NA      <br /> 
 Amount : ₦<?= number_format($amount)  ?><br />
 RRR Code : <?=$rrr  ?><br />
 Date : <?= date('D d M, Y')?>
                        <br /><br />
                        ...
                        <?php 
         $data = $rrr.' '.
                $name.' '.$phone.', '.number_format($amount,2).$email;
        ...
        printf('<img src="%s" alt="QR Code" width="100" height="100"/>', $qrcode);
            ...
            <div class="form-floating mb-3 mt-3">
               <input type="hidden" class="form-control" id="js-rrr" name="rrr" value="<?=$rrr  ?>"/>
...
<script type="text/javascript">
  ...
  processRrr: true,
  transactionId: <?=$rrr  ?>, // Replace with a reference you generated or remove the entire field for us to auto-generate a reference for you. Note that you will be able to check the status of this transaction using this transaction Id
extendedData: {
    customFields: [
       {
          name: "rrr",
